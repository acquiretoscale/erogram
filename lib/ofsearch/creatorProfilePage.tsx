...
import CreatorProfileClient from '@/app/ofsearch/CreatorProfileClient';
import {
...
export async function generateCreatorProfileMetadata(
  ...
  const descTemplates: Record<Locale, string> = {
    en: `${name} OnlyFans profile (@${username}). ${statsSnippet ? `${statsSnippet}. ` : ''}${priceText ? `${priceText}. ` : ''}${socialText}Browse verified OnlyFans creators on Erogram — the #1 OFsearch tool.`,
    de: `${name} OnlyFans-Profil (@${username}). ${statsSnippet ? `${statsSnippet}. ` : ''}${priceText ? `${priceText}. ` : ''}${socialText}Verifizierte OnlyFans Creator auf Erogram entdecken — das #1 OFsearch-Tool.`,
    es: `${name} OnlyFans perfil (@${username}). ${statsSnippet ? `${statsSnippet}. ` : ''}${priceText ? `${priceText}. ` : ''}${socialText}Explora creadoras verificadas en Erogram — el #1 OFsearch.`,
    pt: `${name} OnlyFans perfil (@${username}). ${statsSnippet ? `${statsSnippet}. ` : ''}${priceText ? `${priceText}. ` : ''}${socialText}Explore criadoras verificadas no Erogram — o melhor OFsearch.`,
  };
...
export async function CreatorProfilePageView({
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'OFsearch', item: `${BASE_URL}/ofsearch` },
