...
export default function VickyWidget({ isPremium }: { isPremium: boolean }) {
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className="fixed bottom-5 right-5 w-[72px] h-[72px] rounded-full transition-all duration-200 hover:scale-110 active:scale-95 relative"
        style={{
          zIndex: 9998,
          boxShadow: '0 4px 24px rgba(0,0,0,0.5), 0 0 0 3px rgba(0,175,240,0.4)',
          display: open ? 'none' : 'block',
        }}
      >
        <img src="/assets/vicky-ai-avatar.jpg" alt="Vicky AI" className="w-full h-full rounded-full" style={{ objectFit: 'cover', objectPosition: '50% 65%' }} />
        <span className="absolute top-0 right-0 w-3.5 h-3.5 bg-emerald-400 rounded-full border-2 border-[#111]" />
      </button>
      ...
      {open && (
        <>
          ...
          />
          <div
            className="fixed bottom-4 right-4 left-4 sm:left-auto sm:w-[380px] flex flex-col rounded-2xl overflow-hidden animate-[slideUp_0.2s_ease-out]"
            style={{
              zIndex: 9999,
              height: 'min(520px, calc(100dvh - 80px))',
              background: 'linear-gradient(160deg, #0a0f1e 0%, #0d1628 50%, #0a1220 100%)',
              border: '1px solid rgba(255,255,255,0.12)',
              boxShadow: '0 12px 60px rgba(0,0,0,0.7), 0 0 0 1px rgba(0,175,240,0.15)',
            }}
          >
            {/* Header */}
            ...
            {/* Premium gate with video background */}
            {!isPremium ? (
              <div className="flex-1 relative overflow-hidden" style={{ minHeight: '320px' }}>
                ...
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center px-6 text-center">
                  ...
                  <p className="text-[11px] text-white/50 leading-relaxed mb-5 max-w-[220px] drop-shadow-sm">
                  ...
                  <a href={typeof window !== 'undefined' && localStorage.getItem('token') ? '/welcome' : '/login?redirect=/welcome'}
                    className="px-5 py-2.5 rounded-xl text-[12px] font-bold text-white transition-all hover:brightness-110 shadow-lg"
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
                  {messages.length === 0 && (
                    <div className="flex flex-col items-center pt-6 pb-2">
                      ...
                      <p className="text-[11px] text-white/35 text-center max-w-[240px] mb-4 leading-relaxed">
                        Ask me about the best creators, groups, bots, or AI tools.
                      </p>
                    ...
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} gap-1.5`}>
                      ...
                      <div className={`max-w-[85%] ${msg.role === 'user' ? '' : ''}`}>
                        <div
                          ...
                          <VickyMsg content={msg.content} isUser={msg.role === 'user'} />
                        </div>
...
function VickyMsg({ content, isUser }: { content: string; isUser: boolean }) {
      ...
      {parts.map((part, i) => {
        ...
        if (linkMatch) {
          return <a key={i} href={linkMatch[2]} target="_blank" rel="noopener noreferrer"
            className="text-[#00aff0] underline underline-offset-2 decoration-[#00aff0]/30 hover:decoration-[#00aff0]/60 font-medium">{linkMatch[1]}</a>;
        }
