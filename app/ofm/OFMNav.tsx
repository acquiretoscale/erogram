      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 flex items-center gap-1 h-12">
        {TABS.map((t) => (
          <Link
            key={t.id}
            href={t.href}
            className={`px-4 py-1.5 rounded-lg text-sm font-bold transition ${(active === t.id || (active === 'dashboard' && t.id === 'home')) ? 'bg-[#00AFF0] text-black' : 'text-white/40 hover:text-white/70'}`}
          >
            {t.label}
          </Link>
        ))}
