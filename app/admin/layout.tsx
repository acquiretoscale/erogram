...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(msg === 'Invalid credentials'
        ? 'Invalid email or password. Need an admin user? Run: npx tsx scripts/set-admin-password.ts <email-or-username> <password> true'
        : msg);
      setLoginLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col md:flex-row">
      ...
      <AdminSidebar onLogout={handleLogout} isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />
