...
export default function BotsTab() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (editingBot) {
                ...
                alert('Creating bots from admin panel is not supported. Please use the public interface.');
                return;
            }
    ...
    const filteredBots = bots.filter((bot) =>
        ...
        bot.country.toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black text-white mb-1">Bots</h1>
                    <p className="text-[#999] text-sm">Manage {bots.length} total bots</p>
                </div>
            ...
            <div className="glass rounded-2xl border border-white/5 overflow-hidden">
                {isLoading ? (
                    <div className="p-12 text-center">
                        ...
                        <p className="text-[#999]">Loading bots...</p>
                    </div>
                ) : error ? (
                    <div className="p-12 text-center text-red-400">{error}</div>
                ) : filteredBots.length === 0 ? (
                    <div className="p-12 text-center text-[#999]">No bots found</div>
                ) : (
                    <>
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead className="bg-white/5 border-b border-white/5">
                                    <tr>
                                        <th className="px-6 py-4 text-left text-xs font-bold text-[#666] uppercase tracking-wider">Bot Info</th>
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {paginatedBots.map((bot) => (
                                        <tr key={bot._id} className="hover:bg-white/5 transition-colors group">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <div className="font-medium text-white flex items-center gap-2">
                                                            {bot.name}
                                                            {bot.pinned && <span className="text-[10px] bg-yellow-500/20 text-yellow-500 px-1.5 py-0.5 rounded">PINNED</span>}
                                                        </div>
                                                        <div className="text-xs text-[#666] truncate max-w-[200px]">{bot.telegramLink}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col gap-1">
                                                    <span className="text-sm text-gray-300">{bot.category}</span>
                                                    <span className="text-xs text-[#666]">{bot.country}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${bot.status === 'approved' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                                                    bot.status === 'pending' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' :
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="text-sm text-gray-300">{bot.clickCount?.toLocaleString() || 0} clicks</div>
                                            </td>
                        ...
                        <div className="px-6 py-4 border-t border-white/5 flex items-center justify-between">
                            <div className="text-sm text-[#666]">
                                Showing <span className="text-white font-medium">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-white font-medium">{Math.min(currentPage * itemsPerPage, filteredBots.length)}</span> of <span className="text-white font-medium">{filteredBots.length}</span> results
                            </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => setBotData({ ...botData, status: e.target.value as any })}
                                            className="w-full p-3 bg-[#1a1a1a] border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#b31b1b] outline-none appearance-none"
                                        >
                                    ...
                                    <div className="flex items-center">
                                        <label className="flex items-center gap-3 cursor-pointer">
                                            <input
                                                type="checkbox"
                                                checked={botData.pinned}
                                                onChange={(e) => setBotData({ ...botData, pinned: e.target.checked })}
                                    ...
                                    <div className="col-span-2">
                                        ...
                                        <div className="flex items-center gap-4">
                                            {botData.image && (
                                                <img src={botData.image} alt="Preview" className="w-20 h-20 rounded-lg object-cover border border-white/10" />
                                            )}
                                            <input
                                                type="file"
                                                accept="image/*"
