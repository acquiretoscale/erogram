...
function BotCard({ bot }: { bot: TagBotResult }) {
  const img = bot.image?.startsWith('http') ? bot.image : bot.image || '/assets/image.jpg';
  return (
    <Link
      href={`/${bot.slug}`}
      target="_blank"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-semibold text-[#1a1a1a] group-hover:underline">{bot.name}</p>
        {bot.category && <p className="text-xs text-[#888]">{bot.category}</p>}
        {bot.description && (
          <p className="mt-1 line-clamp-2 text-xs text-[#666]">{bot.description}</p>
        )}
      </div>
    </Link>
  );
...
function AiToolCard({ tool }: { tool: TagAiToolResult }) {
  const img = tool.image?.startsWith('http') ? tool.image : tool.image || '/assets/image.jpg';
  return (
    <Link
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-semibold text-[#1a1a1a] group-hover:underline">{tool.name}</p>
        {tool.category && <p className="text-xs text-[#888]">{tool.category}</p>}
        {tool.description && (
          <p className="mt-1 line-clamp-2 text-xs text-[#666]">{tool.description}</p>
        )}
      </div>
    </Link>
  );
