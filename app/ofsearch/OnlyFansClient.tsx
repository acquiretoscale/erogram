...
import { getTopBestOfByType, BEST_OF_PAGE_MAP, type BestOfPage } from '@/app/best-onlyfans-accounts/bestOfPages';
...
import { OF_SEARCH_TOKENS, ofSearchNavProps } from '@/app/ofsearch/ofSearchTokens';
...
function openCommunityProfile(
  ...
  opts?: { url?: string; hasProfilePage?: boolean },
) {
  onTrack(slug);
  if (opts?.hasProfilePage === false) {
    window.open(ofOutboundUrl(username, opts.url), '_blank', 'noopener,noreferrer');
    return;
  }
...
function CommunityProfileRow({
  ...
  const goAuth = () => {
    if (creator.hasProfilePage === false) {
  ...
  const handleVisit = (e: React.MouseEvent) => {
    ...
    openCommunityProfile(creator.username, creator.slug, onTrack, {
      url: creator.url,
      hasProfilePage: creator.hasProfilePage,
    });
  };
        ...
        {columns.map((col) => (
          ...
          >
            ...
            <ul className="list-none m-0 p-0">
              {col.items.map((cat) => {
                const href = lp(rankingEnglishPublicPath(cat.slug, 'best'));
                const n = rankingListSize(cat.slug);
                const linkText = `${n} Best ${cat.name} OnlyFans Accounts`;
                return (
          ...
          >
            ...
            <ul className="list-none m-0 p-0">
              {group.items.map((page) => {
                const href = lp(rankingEnglishPublicPath(page.slug, 'top'));
                const n = rankingListSize(page.slug);
                const linkText = `Top ${n} ${page.label} OnlyFans Models`;
                return (
...
export default function OnlyFansClient({ initialCreators, totalCreators, initialQuery = '', topBannerCampaigns = [], trendingOnErogram = [], communityCreators = [], topBookmarkedRecent = [], topLikedCreators = [], topLikedPhotos = [], paidFeatured = [] }: Props) {
  ...
  const handleToggleSave = useCallback(async (creatorId: string) => {
    const token = localStorage.getItem('token');
    if (!token) {
      window.open(`/join-erogram?redirect=${encodeURIComponent('/ofsearch')}`, '_blank', 'noopener,noreferrer');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-[#f5f5f5]">
      ...
      <main className="pt-24 sm:pt-28">
        ...
        <section className="bg-gradient-to-b from-[#00AFF0]/10 via-[#00AFF0]/[0.04] to-[#111111] pt-6 pb-8 sm:pt-8 sm:pb-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center mb-6 sm:mb-8">
              ...
              >
                {t('ofSearch.heroTitle')}<span className="text-[#00AFF0]">{t('ofSearch.heroTitleAccent')}</span>
              </motion.h1>
              <motion.p
            ...
            {OF_SEARCH_ENGINE_ENABLED && (
              <ProfileOFPremiumSearch
                ...
                loginRedirect={lp('/ofsearch')}
                paidFeatured={paidFeatured}
                {...ofSearchNavProps(lp)}
              />
            )}
            {OF_SEARCH_ENGINE_ENABLED && !searchActive && (
        ...
        {(OF_SEARCH_ENGINE_ENABLED ? !searchActive : true) && (
          <>
