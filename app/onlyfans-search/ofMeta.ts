...
function ofSeg(locale: Locale): string {
  ...
  return 'onlyfans-search';
}

// ── Canonical URL per locale for each page type ─────────────────────────────

function mainPath(locale: Locale) {
  return locale === 'en' ? '/onlyfans-search' : `/${locale}/${ofSeg(locale)}`;
}
...
function topPath(locale: Locale) {
  return locale === 'en'
    ? '/onlyfans-search/top-creators-2026'
...
export function mainOfMeta(locale: Locale): Metadata {
  ...
  return {
    ...
    alternates: alt('/onlyfans-search', '/de/onlyfans-suche', '/es/onlyfans-busca', current),
    robots,
    openGraph: {
...
// ── Best {label} in 2026 page (/onlyfans-search/blonde2026) ─────────────────
// URL pattern is identical across all locales — just prefixed with /de/ or /es/.

export function best2026OfMeta(locale: Locale, catSlug: string, label: string): Metadata {
  const enPath = `/onlyfans-search/${catSlug}2026`;
  const current = locale === 'en' ? enPath : `/${locale}/${ofSeg(locale)}/${catSlug}2026`;

  const title =
    locale === 'de'
      ? `Beste ${label} OnlyFans 2026 — Top 10 ${label} Creator im Ranking`
      : locale === 'es'
      ? `Mejores ${label} OnlyFans en 2026 — Top 10 Creadoras ${label} en el Ranking`
      : `Best ${label} OnlyFans in 2026 — Top 10 ${label} Creators Ranked`;

  const desc =
    locale === 'de'
      ? `Entdecke die besten ${label} OnlyFans Creator 2026. Unser Top-10-Ranking basiert auf echten Klicks, Trending-Signalen und Community-Popularität — täglich aktualisiert.`
      : locale === 'es'
      ? `Descubre las mejores creadoras ${label} de OnlyFans en 2026. Nuestro ranking top 10 se basa en clics reales, señales de tendencia y popularidad — actualizado diariamente.`
      : `Discover the best ${label} OnlyFans creators in 2026. Our top 10 ranking is based on real clicks, trending signals, and community popularity — updated daily.`;

  const ogTitle =
    locale === 'de'
      ? `Beste ${label} OnlyFans 2026 | Erogram`
      : locale === 'es'
      ? `Mejores ${label} OnlyFans 2026 | Erogram`
      : `Best ${label} OnlyFans in 2026 | Erogram`;

  const twDesc =
    locale === 'de'
      ? `Top 10 ${label} OnlyFans Creator 2026 — gerankt nach Klicks und Popularität.`
      : locale === 'es'
      ? `Top 10 creadoras ${label} de OnlyFans 2026 — clasificadas por clics y popularidad.`
      : `Top 10 ${label} OnlyFans creators in 2026 — ranked by clicks and popularity.`;
  ...
  return {
    ...
    alternates: alt(
      enPath,
      `/de/onlyfans-suche/${catSlug}2026`,
      `/es/onlyfans-busca/${catSlug}2026`,
      current,
    ),
    robots,
    openGraph: {
