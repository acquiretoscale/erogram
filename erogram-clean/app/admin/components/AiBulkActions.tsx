...
const REWRITE_SYS = `You are a concise SEO content writer for Erogram.pro — an adult NSFW Telegram groups directory.\n\n${EROGRAM_CTX}\n\nCRITICAL: You MUST write ALL descriptions in ENGLISH regardless of the source language.\n\nRules:\n- ALWAYS output in ENGLISH\n- Each description MUST be 100-120 characters. NO MORE than 120 chars. NO LESS than 100 chars.\n- Structure: "[Niche] Telegram group with [X]+ members. [One factual sentence about content/focus]." If country is known, mention it.\n- Include member count if available (e.g. "5K+ members")\n- Include country of origin if detectable from original description or group name\n- Keep any factual, non-spammy detail from the original (e.g. posting frequency, content type)\n- NO filler (no "Join now!", "Best group!", "Don't miss!", "Explore", "Discover")\n- NO fluff adjectives (no "amazing", "incredible", "ultimate", "exclusive")\n- Be factual and direct — like a directory listing, not marketing copy\n- Non-Latin names: keep original + [English meaning]\n- Return ONLY [N] rewritten_text format`;

const REWRITE_USR = `Rewrite these {{count}} group descriptions IN ENGLISH. STRICT LENGTH: 100-120 characters each — count carefully. Include: niche, member count, category, country (if known). No fluff, no filler, factual only. Return each on its own line in format [N] rewritten_text:\n\n{{groups}}`;
...
const TR_SYS = (lang: string) => `You are a native ${lang} speaker writing for Erogram.pro — an adult NSFW Telegram directory.\n\n${EROGRAM_CTX}\n\nTranslation rules:\n- Translate naturally in ${lang} as a native speaker\n- STRICT LENGTH: 100-120 characters each — no more, no less\n- Keep it factual and concise — niche, members, category, country\n- NO filler words, NO fluff adjectives\n- Preserve brand names and group names\n- Non-Latin names: keep original + [English meaning]\n- Return ONLY [N] translated_text format`;

const TR_USR = `Translate these {{count}} group descriptions to {{language}}. STRICT LENGTH: 100-120 characters each. Factual, concise, no filler. Return each on its own line in format [N] translated_text:\n\n{{groups}}`;
